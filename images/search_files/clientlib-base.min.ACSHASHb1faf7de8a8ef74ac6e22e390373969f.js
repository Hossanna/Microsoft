...
$(document).ready(function() {
    $('div[clickgroup-telemetry-enabled]').each(function() {
        var anchorCount = $(this).find('div.link-group a').length;
        if (anchorCount === 1) {
          $(this).css('cursor', 'pointer');
        }
     });

     $('li[clickgroup-telemetry-enabled]').each(function() {
        var anchorCount = $(this).find('li.link-group a').length;
        if (anchorCount === 1) {
          $(this).css('cursor', 'pointer');
        }
     });

    $('div[clickgroup-telemetry-enabled]').click(function(e) {
        if ($(e.target).closest('div.link-group a').length) {
        // Code to execute when the anchor tag is clicked
            let currentElement =$(e.target).closest('div.link-group a');
            if ( currentElement && currentElement.html() && !currentElement.data( "telemetry" ) ) {
        ...
        function retrieveCurrentElement(e) {
            if ($(e.target).closest("div[clickgroup-telemetry-enabled]").html()) {
                if ($(e.target).closest("section[data-mount='click-group']").length > 0) { 
                    let currentElement = $(e.target).closest("section[data-mount='click-group']") ;
                    currentElement = currentElement && !currentElement.html() ? $(e.target).closest("div[clickgroup-telemetry-enabled]") : currentElement;
    ...
    $('li[clickgroup-telemetry-enabled]').click(function(e) {
        if ($(e.target).closest('li.link-group a').length) {
        // Code to execute when the anchor tag is clicked
            let currentElement =$(e.target).closest('li.link-group a');
            if ( currentElement && currentElement.html() && !currentElement.data( "telemetry" ) ) {
                currentElement.data( "telemetry", true );
                captureTelemetryPageAction.call(this, currentElement.data());
            }
        ...
        if (!$(e.target).closest('li.link-group a').length) {
        // Code to execute when the li (excluding the anchor tags) is clicked
            let currentElement = retrieveCurrentElement(e);
            if ( currentElement && currentElement.html() && !currentElement.data( "telemetry" ) ) {
                currentElement.data( "telemetry", true );
                let linkGroup = currentElement.find("a") || currentElement.find(".link-group");
                ...
                // Redirect if there is only one anchor tag
                if (anchorCount === 1) {
                  // Get the href of the first anchor tag inside the li
                  var anchorHref = linkGroup[0].getAttribute('href');
                  var target = linkGroup[0].getAttribute('target');
                  if (target != '_blank') {
                  // Redirect the page to the same path as the anchor's href if target is not _blank
        ...
        function retrieveCurrentElement(e) {
            if ($(e.target).closest("li[clickgroup-telemetry-enabled]").html()) {
                if ($(e.target).closest("section[data-mount='click-group']").length > 0) { 
                    let currentElement = $(e.target).closest("section[data-mount='click-group']") ;
                    currentElement = currentElement && !currentElement.html() ? $(e.target).closest("li[clickgroup-telemetry-enabled]") : currentElement;
                    return currentElement;
                }
            }
        }
...
$(()=>{
    document.addEventListener('click', (e)=>{
        if (e.target.matches('moray-anchor')) {
